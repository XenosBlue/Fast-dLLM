...
import time
import types
import layer_skip_generation_functions
from layer_skip_generation_functions import apply_layer_skipping
...
_real_load = _datasets.load_dataset
_SHA1 = re.compile(r"^[0-9a-f]{40}$")

def _forced_patch_load(path, *args, **kwargs):
    orig_path = path
    ...
    rev = kwargs.get("revision", None)
    if isinstance(rev, str) and _SHA1.match(rev):
        kwargs["revision"] = "main"
    kwargs.pop("trust_remote_code", None)

    if orig_path != path or "revision" in kwargs:
        print(f"⚡ DATASET PATCH: {orig_path} -> {path} | revision={kwargs.get('revision', None)}")

    return _real_load(path, *args, **kwargs)

_datasets.load_dataset = _forced_patch_load
...
class Fast_dLLM_v2EvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        generation_functions.apply_layer_skipping(self.model, cosine_threshold=self.cosine_threshold)
        self.model.eval()
    ...
    def loglikelihood(self, requests):
        ...
        out = []
        with torch.no_grad():
            if hasattr(self.model, 'layer_skip_state'):
                self.model.layer_skip_state['total_flops'] = 0.0
        ...
        if hasattr(self.model, 'layer_skip_state'):
             tflops = self.model.layer_skip_state['total_flops']
             print(f"\n[BENCHMARK] LogLikelihood Task | Total TFLOPs: {tflops}\n")
